...
export default function QuestionBankExcelUpdate() {
  ...
  const handlePreview = async () => {
    if (!file) {
      message.warning('Excel 파일을 선택해주세요.');
      return;
    }
    setLoading(true);
    try {
      const res = await questionBankApi.excelUpdatePreview(file);
      if (res.success) {
  ...
  const handleApply = async () => {
    ...
    setLoading(true);
    try {
      const res = await questionBankApi.excelUpdateApply(file);
      if (res.success) {
  ...
  return (
    <div>
      <Title level={4}>Excel 정답/배점/난이도 업데이트</Title>
      ...
      {currentStep === 0 && (
        <Card>
            ...
            description={
              <div>
                <p style={{ margin: '4px 0' }}>
                  헤더(첫 행): <strong>시험코드 | 시험명 | 회차 | 과목명 | 문항번호 | 정답 | 배점 | 난이도</strong>
                </p>
                <p style={{ margin: '4px 0' }}>
                  시험코드와 과목명으로 그룹코드(<code>시험코드-과목명</code>)를 생성하여 매칭합니다.
                </p>
              </div>
            }
            type="info"
          ...
          >
            ...
            <p className="ant-upload-text">Excel 파일을 드래그하거나 클릭하여 선택</p>
